...
export function buildWhatsAppMessage(
  ...
  for (const item of items) {
    ...
    total += subtotal;
    lines.push(
      `• ${name} x${item.quantity} — ${formatPrice(subtotal, business.currency)}`,
    );
  }
  ...
  if (business.address) {
    ...
    lines.push(`${addrLabel} ${business.address}`);
  }
...
export function buildDonationQRText(
  business: BusinessDetail,
  lang: Lang = "es",
): string {
  const prefix = "TRANSFERMOVIL_ETECSA,TRANSFERENCIA";
  const card = business.cardNumber || "";
  const phone = business.phone || "";
  return `${prefix},${card},${phone}`;
}
