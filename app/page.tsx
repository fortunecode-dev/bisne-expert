...
export default function HomePage() {
  ...
  const [details, setDetails] = useState<Record<string, BusinessDetail>>({});
  ...
  useEffect(() => {
    Promise.all([
      import("@/data/businesses.json").then((m) => m.default),
      import("@/data/config.json").then((m) => m.default),
    ]).then(([bizData, cfg]) => {
      setConfig(cfg);
      if ((cfg as any).homePalette) applyHomeTheme((cfg as any).homePalette);

      const bizList = (bizData as any).businesses as Business[];
      setBusinesses(bizList.filter((b: Business) => !b.hidden));

      Promise.allSettled(
        bizList.map((b: Business) =>
          import(`@/data/business/${b.slug}.json`)
            .then((m) => ({
              slug: b.slug,
              detail: m.default as BusinessDetail,
            }))
            .catch(() => null),
        ),
      ).then((results) => {
        const map: Record<string, BusinessDetail> = {};
        results.forEach((r) => {
          if (r.status === "fulfilled" && r.value)
            map[r.value.slug] = r.value.detail;
        });
        setDetails(map);
      });
    });
