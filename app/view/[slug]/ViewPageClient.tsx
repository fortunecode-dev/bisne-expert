...
const EMOJI_MAP: Record<string, string> = { 'burger-house': '🍔', 'pizza-palace': '🍕' }
...
export function ViewPageClient({ business, detail, products, slug }: Props) {
  ...
  const { items, add, remove, update, clear, setFromParam, total, count, cartParam, mounted } = useCart(slug, indexes.productsById)
  ...
  useEffect(() => {
    if (!loaded && mounted) {
      ...
      if (c) {
        ...
        const cartItems: CartItem[] = parsed.map(({ product, quantity }) => ({ productId: product.id, quantity }))
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--color-bg)' }}>
      <header className="sticky top-0 z-30 border-b border-white/5 backdrop-blur-md" style={{ background: 'rgba(15,15,15,0.9)' }}>
        <div className="max-w-xl mx-auto px-4 h-16 flex items-center gap-3">
          <Link href={'/' + 'business/' + slug} className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-white/10 transition-colors">←</Link>
          <span className="font-bold flex-1" style={{ fontFamily: 'var(--font-display)' }}>
      ...
      <div className="max-w-xl mx-auto px-4 py-6">
        <div className="rounded-2xl overflow-hidden border border-white/5" style={{ background: 'var(--color-surface)' }}>
          <div className="px-5 py-4 border-b border-white/5 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs" style={{ color: 'var(--color-text-muted)' }}>
                {count} {lang === 'es' ? 'items en el carrito' : 'items in cart'}
              </p>
            </div>
            <button
              onClick={handleCopy}
